      ...
      if (result.data && result.data.length > 0) {
        ...
        const features = result.data.map((item: any) => {
          ...
          const feature = new Feature({
            geometry: new Point(fromLonLat([item.longitude ? item.longitude : 0, item.latitude ? item.latitude : 0])), // 使用经纬度创建点几何
    ...
    map.current.on('pointermove', function (evt: any) {
      ...
      map.current.forEachFeatureAtPixel(evt.pixel, (feature: any, layer: any) => {
        if (layer !== geoJSONLayer) {          
          ...
          popupContentRef.current.innerHTML += `<p class=${styles.olPopupContentValue}>${feature.getProperties()?.carbonTotal ? feature.getProperties()?.carbonTotal.toFixed(2) : 0}t</p>`
          // 弹出popup
