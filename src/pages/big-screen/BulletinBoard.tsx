...
import { ConfigProvider, DatePicker, Segmented, Space, Table } from "antd"
...
import { datePickerEnum, elasticEnergyColumns, INTERVALTIME, monitorColumns, statusQuoChartOptions } from './utils'
import dayjs from 'dayjs';
import 'dayjs/locale/zh-cn'; // 引入中文语言包
import { useRequest } from 'ahooks'
import { getBoardCenterData, getElasticEnergyManage, getEnergyManageFeature, getEnergyTrend, getEnterpriseEnergyMonitor, getStatusQuo } from '@/services/big-screen'
...
const BulletinBoard = () => {
  ...
  // 区域用能概览 --- 特征
  const { data: energyFeayureData, run: fetchEnergyManageFeature } = useRequest(getEnergyManageFeature, {
    manual: true,
    pollingInterval: INTERVALTIME,
    pollingErrorRetryCount: 3,
  });
  ...
  const getEnergyOverviewOptions = () => {
    switch (currentView) {
      ...
      case '特征':
        return { currentView, data: pageDataHandle(energyFeayureData), type: fullAndPutDate }
    }
  }
                    ...
                    <div className={styles.energyOverviewSearch}>
                      <Space align="center" >
                        ...
                        <SegmentedTheme
                          ...
                          options={currentView == '特征' ? ['月', '年'] : datePickerEnum.map((item: any) => item.name)}
                        />
                      </Space>
                    </div>
                }
