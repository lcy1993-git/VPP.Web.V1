...
const AddCollapse = (props: any) => {
  ...
  const [collapseItems, setCollapseItems] = useState([
    {
      key: '1',
      label: <CollapseLabelRow label={`collapse1`} />,
  ...
  const saveData = (currentIndex: any) => {
    ...
    // 确保所有输入都有值
    const allFilled = tableData.every((item: any) => item.declaredCapacity && item.declaredPrice);
    if (allFilled) {
      // 处理表格编辑状态
      setTableInputDisable({
        ...tableInputDisable,
  ...
  return (
    <div className={styles.addDeclarePanel}>
      <MyProvider
        value={{
      ...
      >
        <div className={styles.addDeclarePanelMain}>
          <Collapse items={collapseItems} collapsible="icon" defaultActiveKey={'1'} />
        </div>
