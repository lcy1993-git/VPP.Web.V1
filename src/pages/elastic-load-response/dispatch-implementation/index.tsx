...
import {
  ...
  getPlanInfo,
  getRealList,
} from '@/services/elastic-load-response/dispatch-implementation';
...
import {
  alarmTableColumns,
  executionStatusDataSource,
  executionStatusDataType,
  executionStatusTableColumns,
  planChartOptions,
  planInfoType,
  statusticsChart,
} from './utils';
...
const DispatchImplementation = () => {
  ...
  // 执行情况数据
  const [executionStatusData, setexecutionStatusData] = useState<executionStatusDataType | null>(
    null,
  );
  // 日前邀约计划列表数据
  const [realList, setRealList] = useState<{
    realTimeDispatchPlan: string[];
    dayAheadInvitationPlan: string[];
  }>({ realTimeDispatchPlan: [], dayAheadInvitationPlan: [] });
  ...
  // 请求执行情况
  const { run: fetchExecutionStatus, loading: executionStatusLoading } = useRequest(
    getExecutionStatus,
    {
      manual: true,
      onSuccess: (result) => {
                ...
                <div className={styles.listItemContent}>
                  ...
                  <span className={styles.column}>
                    ...
                    <span className={styles.value}>
                      {historyData?.dayAheadTransactionPeakShaving}
                    </span>
                  </span>
                  <span className={styles.column}>
                    <span className={styles.label}>填谷：</span>
                    <span className={styles.value}>
                      {historyData?.dayAheadTransactionValleyFilling}
                    </span>
                  </span>
                </div>
              </div>
              <div className={styles.cardListItem}>
                ...
                <div className={styles.listItemContent}>
                  ...
                  <span className={styles.column}>
                    <span className={styles.label}>削峰：</span>
                    <span className={styles.value}>
                      {historyData?.realTimeTransactionPeakShaving}
                    </span>
                  </span>
                  <span className={styles.column}>
                    <span className={styles.label}>填谷：</span>
                    <span className={styles.value}>
                      {historyData?.realTimeTransactionValleyFilling}
                    </span>
                  </span>
                </div>
              </div>
              <div className={styles.cardListItem}>
                ...
                <div className={styles.listItemContent}>
                  ...
                  <span className={styles.column}>
                    <span className={styles.label}>削峰：</span>
                    <span className={styles.value}>
                      {historyData?.realTimeTransactionRevenuePeakShaving}
                    </span>
                  </span>
                  <span className={styles.column}>
                    <span className={styles.label}>填谷：</span>
                    <span className={styles.value}>
                      {historyData?.realTimeTransactionRevenueValleyFilling}
                    </span>
                  </span>
                </div>
              </div>
              <div className={styles.cardListItem}>
                ...
                <div className={styles.listItemContent}>
                  ...
                  <span className={styles.column}>
                    <span className={styles.label}>削峰：</span>
                    <span className={styles.value}>
                      {historyData?.dayAheadTransactionRevenuePeakShaving}
                    </span>
                  </span>
                  <span className={styles.column}>
                    <span className={styles.label}>填谷：</span>
                    <span className={styles.value}>
                      {historyData?.dayAheadTransactionRevenueValleyFilling}
                    </span>
                  </span>
                </div>
              </div>
      ...
      <div className={styles.planList}>
        <CustomCard>
          <div className={styles.planDate}>
            <DatePicker defaultValue={dayjs(new Date())} onChange={datePickerChange} />
          </div>
          <div className={styles.planContent}>
            <div className={styles.planContentWrap}>
                    ...
                    {(realList?.dayAheadInvitationPlan || []).map((item) => {
                      return (
                        <p
                          ...
                          className={`${
                            [
                              ...realList?.realTimeDispatchPlan,
                              ...realList?.dayAheadInvitationPlan,
                            ].includes(currentInvitationId)
                              ? styles.listActive
                              : null
                          }`}
              ...
              <div className={styles.planInfo}>
                <div className={styles.planInfoTitle}>计划信息：{planInfo?.identificationNum}</div>
                <div className={styles.planInfoBasic}>
                  ...
                  <p>参与用户：{planInfo?.numParticipatingUsers}家</p>
