...
import { getCompanyCurve, getTrackingCure, getTrackingPlanList } from '@/services/elastic-load-response/components';
...
import { companyChartOptions, companyTableColumns, companyTableData, dayPlanChartOptions, dayPlanTableColumns, dayPlanTableData, sourceTableColumns } from '../utils';
...
const ExecutionTracking = () => {
  ...
  // 请求执行情况
  const { run: fetchTrackingPlanList, loading: trackingPlanListLoading } = useRequest(getTrackingPlanList, {
    manual: true,
    onSuccess: (result) => {
      ...
      if (code === 200 && data) {
        ...
        const dayPlanData = dayAheadInvitationPlan.map((item: any, index: number) => {
          return {
            ...
            children: <ul className={styles.planChildren}>
              <li><span>计划开始时间：</span><span>{item.startTime.split(' ')[0]}</span></li>
              <li className={styles.borderBottom}><span>计划结束时间：</span><span>{item.endTime.split(' ')[0]}</span></li>
              <li><span>目前计划里程：</span><span>{item.planMilestone}MWh</span></li>
              <li><span>实时完成里程：</span><span>{item.actualMilestone}MWh</span></li>
              <li><span>参与用户：</span><span>{item.participatingUsers}家</span></li>
              <li><span>功率偏差：</span><span>{item.powerDeviation}%</span></li>
              <li><span>预计收益：</span><span>{item.estimatedEarnings}万元</span></li>
            </ul>
          }
        })
        const realtimePlanData = realTimeDispatchPlan.map((item: any, index: number) => {
          return {
            ...
            label: item.identificationNum,
            children: <ul className={styles.planChildren}>
              <li className={styles.borderBottom}><span>指令下发时间：</span><span>{item.commandIssuingTime.split(' ')[0]}</span></li>
              <li><span>调度开始时间：</span><span>{item.startTime.split(' ')[0]}</span></li>
              <li><span>调度结束时间：</span><span>{item.endTime.split(' ')[0]}</span></li>
              <li><span>调度计划里程：</span><span>{item.planMilestone}MWh</span></li>
              <li><span>实际调控里程：</span><span>{item.actualMilestone}MWh</span></li>
              <li><span>参与用户：</span><span>{item.participatingUsers}家</span></li>
              <li><span>资源在线状态：</span><span>{item.resourceOnline}台</span></li>
              <li><span>功率偏差：</span><span>{item.powerDeviation}%</span></li>
              <li><span>预计收益：</span><span>{item.estimatedEarnings}万元</span></li>
            </ul>
          }
